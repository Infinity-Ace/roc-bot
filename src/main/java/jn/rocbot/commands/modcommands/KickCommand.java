...
public class KickCommand implements Command{
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        if(args.length == 0){
            event.getTextChannel().sendMessage(String.format("You must specify someone to be kicked, use `%shelp kick` for more details", PREFIXES.NORMAL.PREFIX)).complete();
            return;
        }
        ...
        if(joined.toString().contains("|"))
            message = String.format(
                    "You have been kicked by a moderator, reason: %s",
                    joined.toString().substring(
                            joined.toString().indexOf("|") + 1
                    ).replace("|", "")
            ).trim();
        else message = "You have been kicked by a moderator";

        try {
            if (event.getGuild().getMemberById(Long.valueOf(user)) != null)
                kick(event.getGuild().getMemberById(Long.valueOf(user)), message);
    ...
    public String help() {
        return String.format(
                "Usage: %skick Username/Nickname/user-longId [|Reason]\n" +
